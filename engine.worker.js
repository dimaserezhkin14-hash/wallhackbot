...
function generateCandidates(turnPlayer,walls,myPos,oppPos,myWalls,oppWalls,limit=ENGINE.nodeWallLimit,searchDepth=1){
  ...
  /* Root-level completeness pass. Inspect every geometrically legal wall,
     but do only two BFS passes per wall. Expensive fork analysis is reserved
     for the small set that already looks tactically relevant. */
  const beforeInfo=shortestPathInfo(enemy,walls,ePos);
  const beforeProgress=immediateProgressCount(enemy,walls,ePos,pPos);
  if(!ARENA_FAST && searchDepth<=1 && pWalls>0 && (beforeProgress>=2 || beforeInfo.ways<=2 || Math.abs(beforeInfo.dist-curP)<=2)){
    const existing=new Set(all.map(moveKey));
    for(let rr=0; rr<=getBoardRows()-2; rr++){
      for(let cc=0; cc<8; cc++){
        for(const oo of ["h","v"]){
          if((engineStats.nodes&31)===0 && performance.now()>=engineDeadline){
            engineAborted=true; break;
          }
          if(!wallGeometryLegal(rr,cc,oo,walls)) continue;
          const w={r:rr,c:cc,o:oo};
          const k=moveKey({type:"wall",...w});
          if(existing.has(k)) continue;
          const copy=new Set(walls); copy.add(wallKey(rr,cc,oo));
          const ei=shortestPathInfo(enemy,copy,ePos);
          const pi=shortestPathInfo(turnPlayer,copy,pPos);
          if(!isFinite(ei.dist)||!isFinite(pi.dist)) continue;
          const progressAfter=immediateProgressCount(enemy,copy,ePos,pPos);
          const eGain=ei.dist-beforeInfo.dist;
          const pLoss=pi.dist-curP;
          const wayDrop=beforeInfo.ways-ei.ways;
          const progDrop=beforeProgress-progressAfter;
          if(eGain>0 || wayDrop>0 || progDrop>0 || pLoss<=0){
            let critical=wayDrop*2600+eGain*4200+progDrop*1800;
            if(wayDrop>0 || progDrop>0) critical+=criticalWallScore(turnPlayer,walls,pPos,ePos,w);
            all.push({type:"wall",r:rr,c:cc,o:oo,priority:critical,_eGain:eGain,_pLoss:pLoss,_critical:critical});
            existing.add(k);
          }
        }
  ...
  const pathCheck=Math.min(48,all.length);
...
function tacticalPositionScore(walls,myPos,oppPos,myWalls,oppWalls){
  ...
  if(op.dist<me.dist){
    score-=2200*(me.dist-op.dist);
    if(op.ways>=2) score-=900;
  }
...
function rootMoveIsSafe(m){
  ...
  for(const s of opSteps){
    ...
    if(d < opDist && d+1 < meDist-1) return false;
  }
  /* Also reject a root move that leaves the opponent with a cheap wall
     fork: two shortest exits, with one wall capable of killing either our
     lane or the opponent's lane. */
  if(state.opp.walls>0){
    const opInfo=shortestPathInfo("opp",n.walls,n.oppPos);
    const opProg=immediateProgressCount("opp",n.walls,n.oppPos,n.myPos);
    if(opProg>=2 && opInfo.dist<=meDist+1) return false;
  }
  return true;
}
...
function calculateBestMove(player){
  ...
  if(searchRoot.length>1){
    ...
    for(const m of searchRoot){ m._policy=(pol[spnnActionIndex(m)]||0); m.priority += m._policy*180; }
...
self.onmessage = function(ev){
  ...
  try{
    ...
    self.postMessage({type:"result",requestId:msg.requestId,move,stats:{...engineStats}});
  }catch(err){
