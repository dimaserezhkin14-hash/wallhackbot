...
function generateSearchMoves(s,turn,depth){
  ...
  out.sort((a,b)=>b.priority-a.priority);
  if(depth<=2 && out.length>40){
    const pawn=out.filter(m=>m.type==="step");
    const walls=out.filter(m=>m.type==="wall");
    const keep=Math.min(walls.length, depth===1 ? 28 : 40);
    out.length=0;
    for(const m of pawn) out.push(m);
    for(let i=0;i<keep;i++) out.push(walls[i]);
  }
  return out;
}
...
function openingPenalty(s,m,turn){
  // Soft opening principle: in a quiet starting position, movement is preferred
  // over spending a wall unless the wall creates a concrete tactical gain.
  const p=turn==="me"?s.myPos:s.oppPos;
  const e=turn==="me"?s.oppPos:s.myPos;
  const quiet=s.walls.size===0 && p.r===(turn==="me"?getBoardRows()-1:0) && e.r===(turn==="me"?0:getBoardRows()-1);
  if(!quiet || m.type!=="wall") return 0;
  return -1800;
}
function evaluate(s){
...
function calculateBestMove(){
  ...
  const start=now();
  // Opening-book principle: with an empty board and the standard starting
  // position, immediate central advancement is the dominant practical move.
  // This is not a hard prohibition on walls in general; it is the same kind
  // of tiny book an engine uses to avoid spending milliseconds proving an
  // uncontested first move.
  if(state.walls.size===0 && state.myPos.c===4 && state.oppPos.c===4 &&
     state.myPos.r===getBoardRows()-1 && state.oppPos.r===0){
    const forward={r:state.myPos.r-1,c:state.myPos.c};
    if(legalMoves("me",state.walls,state.myPos,state.oppPos).some(x=>x.r===forward.r&&x.c===forward.c)){
      completedDepth=0;
      return {type:"step",r:forward.r,c:forward.c,priority:0};
    }
  }
