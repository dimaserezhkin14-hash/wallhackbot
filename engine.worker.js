...
function edgeBlocked(a,b,walls){
  ...
  deadline=savedDeadline; return true;
}
...
function legalWall(r,c,o,walls,myPos,oppPos){
  ...
  // Quoridor permits adjacent walls of the same orientation.  The only
  // geometric overlap that is illegal is crossing at the same slot.
  if(o==="h"){
...
function wallPlanThreat(s,depth=2){
  ...
  for(const fw of first){
    ...
    const immediate=sig1.d-sig1.d + Math.max(0,sig1.tunnel-1)*1600 + Math.max(0,8-sig1.paths)*450;
...
function evaluate(s){
  ...
  v += (mySig.paths-oppSig.paths)*140;
  v += (mySig.exits-oppSig.exits)*180;
  v += (oppSig.tunnel-mySig.tunnel)*1150;
  v += (oppSig.choke-mySig.choke)*280;
...
function calculateBestMove(){
  ...
  const initial = opening && state.myPos.r===getBoardRows()-1 && state.myPos.c===4 && state.oppPos.r===0 && state.oppPos.c===4;
  if(initial) return {type:"step",r:state.myPos.r-1,c:state.myPos.c,priority:0};
  const critical=(state.oppWalls>=5 || state.myWalls<=2 || md<=4 || od<=4 || Math.abs(md-od)<=2);
  const budget = opening ? 180 : (totalWalls<=1 ? 650 : (totalWalls<=3 ? 420 : (critical ? 320 : 220)));
  const searchStart=now();
  const generationBudget=Math.min(80, Math.max(35, Math.floor(budget*0.18)));
  ...
  if(state.oppWalls>0 && (od<=7 || md<=7 || Math.abs(md-od)<=2 || state.walls.size>=2)){
    const screened=[];
    for(const m of root){
      if(expired()){timedOut=true;break;}
      const child=applyMove(state,m,true);
      const future=wallPlanThreat(child,2);
      const sigMe=routeSignature("me",child.walls,child.myPos,child.oppPos);
      const sigOpp=routeSignature("opp",child.walls,child.oppPos,child.myPos);
      const planPenalty=future*0.55 + sigOpp.tunnel*900 + Math.max(0,4-sigOpp.exits)*300;
  ...
  let best=apkFast || root.find(m=>m.type==="step") || root[0];
