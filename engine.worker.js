...
/* ============================================================
   EXACT ZERO-WALL SOLVER
   When no walls remain the game graph is small enough to solve exactly.
   This is deliberately a retrograde game-theoretic solver, not a depth
   limit.  It also classifies cycles as draws, so the engine can NEVER get
   trapped in an endless pawn-move search in a wall-less position.
   ============================================================ */
let ZERO_WALL_CACHE = null;

function zeroWallIndex(p1,p2,turn,cellCount){
  return ((turn*cellCount + p1)*cellCount + p2);
}

function buildZeroWallSolver(){
  const rows=getBoardRows(), cells=rows*9, N=2*cells*cells;
  const result=new Int8Array(N); result.fill(2); // 2 = unresolved, 0 = draw
  const remaining=new Uint8Array(N);
  const pred=[...Array(N)];
  const queue=new Int32Array(N);
  let qh=0,qt=0;

  const cellPos=i=>({r:(i/9)|0,c:i%9});
  const goalMe=getGoalRow('me'), goalOpp=getGoalRow('opp');

  for(let p1=0;p1<cells;p1++){
    const a=cellPos(p1);
    for(let p2=0;p2<cells;p2++){
      if(p1===p2) continue;
      const b=cellPos(p2);
      for(let turn=0;turn<2;turn++){
        const id=zeroWallIndex(p1,p2,turn,cells);
        if(a.r===goalMe){ result[id]=1; queue[qt++]=id; continue; }
        if(b.r===goalOpp){ result[id]=-1; queue[qt++]=id; continue; }

        const moves=turn===0 ? legalMoves('me',new Set(),a,b) : legalMoves('opp',new Set(),b,a);
        remaining[id]=moves.length;
        for(const m of moves){
          const np=m.r*9+m.c;
          const succ=turn===0
            ? zeroWallIndex(np,p2,1,cells)
            : zeroWallIndex(p1,np,0,cells);
          if(!pred[succ]) pred[succ]=[];
          pred[succ].push(id);
        }
        if(moves.length===0){ result[id]=0; queue[qt++]=id; }
      }
    }
  }

  while(qh<qt){
    const solved=queue[qh++], rw=result[solved];
    const list=pred[solved];
    if(!list) continue;
    for(const parent of list){
      if(result[parent]!==2) continue;
      const parentTurn = parent < cells*cells ? 0 : 1;
      const mover = parentTurn===0 ? 1 : -1;
      if(rw===mover){
        // Side to move can choose this winning successor.
        result[parent]=mover;
        queue[qt++]=parent;
      }else if(rw===-mover){
        // Count only successors proven winning for the opponent. A draw or
        // unresolved/cyclic successor must NOT make the parent look lost.
        if(remaining[parent]>0) remaining[parent]--;
        if(remaining[parent]===0){
          // Every successor is winning for the opponent.
          result[parent]=-mover;
          queue[qt++]=parent;
        }
      }
  ...
  ZERO_WALL_CACHE={rows,cells,result};
  return ZERO_WALL_CACHE;
}

function zeroWallOutcome(p1,p2,turn){
  const z=ZERO_WALL_CACHE || buildZeroWallSolver();
  const id=zeroWallIndex(p1,p2,turn,z.cells);
  return z.result[id]===2 ? 0 : z.result[id];
}

function exactZeroWallMove(){
  const p1=state.me.r*9+state.me.c, p2=state.opp.r*9+state.opp.c;
  const moves=legalMoves('me',new Set(),state.me,state.opp);
  if(!moves.length) return null;
  let best=null, bestOutcome=-2;
  for(const m of moves){
    const np=m.r*9+m.c;
    const out=zeroWallOutcome(np,p2,1);
    // After our move, the position is winning for us iff its outcome is 1.
    if(out===1) return m.type==='step'?{type:'step',r:m.r,c:m.c}:m;
    if(out>bestOutcome){bestOutcome=out;best=m;}
  }
  // If no forced win exists, never choose a move known to lose when a draw exists.
  if(bestOutcome>=0) return best;
  return best || moves[0];
}

function resetZeroWallCache(){ ZERO_WALL_CACHE=null; }
...
/* ============================================================
   ...
   ============================================================ */
const MONSTER = { mctsMs: 140, maxChildren: 48, endgameWalls: 5, exactDepth: 40, maxExactNodes: 180000 };
...
let exactNodes=0;
function monsterExactEndgame(depth, maximizing, walls,myPos,oppPos,myWalls,oppWalls,alpha,beta,turn,rootPlayer,seen,stack){
  if((++exactNodes % 256)===0 && performance.now()>=engineDeadline){ engineAborted=true; return 0; }
  if(exactNodes>=MONSTER.maxExactNodes){ engineAborted=true; return 0; }
  ...
  if(depth<=0) return evaluateStateFull('me',walls,myPos,oppPos,myWalls,oppWalls);
  const key=initialWallHash(walls).join(':')+'|'+myPos.r+','+myPos.c+'|'+oppPos.r+','+oppPos.c+'|'+myWalls+','+oppWalls+'|'+turn;
  // Re-entering the same position before a terminal result is a cycle.
  // Treat it as neutral rather than searching the same pawn loop forever.
  if(stack.has(key)) return 0;
  const cached=seen.get(key);
  if(cached && cached.depth>=depth) return cached.value;
  stack.add(key);
  const moves=monsterMoveList(turn,walls,myPos,oppPos,myWalls,oppWalls,turn==='me'?24:24);
  if(!moves.length){ stack.delete(key); return evaluateStateFull('me',walls,myPos,oppPos,myWalls,oppWalls); }
  let best=maximizing?-Infinity:Infinity;
  for(const m of moves){
    const n=monsterStateAfter(m,walls,myPos,oppPos,myWalls,oppWalls,turn); if(!n) continue;
    const v=monsterExactEndgame(depth-1,!maximizing,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls,alpha,beta,turn==='me'?'opp':'me',rootPlayer,seen,stack);
    if(engineAborted){ stack.delete(key); return 0; }
    if(maximizing){best=Math.max(best,v);alpha=Math.max(alpha,v);}else{best=Math.min(best,v);beta=Math.min(beta,v);}
    if(beta<=alpha) break;
  }
  stack.delete(key);
  seen.set(key,{depth,value:best});
  return best;
}


function rootTrapRisk(m){
  const n=applyMoveForSearch(m,state.walls,state.me,state.opp,state.me.walls,state.opp.walls,0,0,'me');
  if(!n) return -Infinity;
  if(n.oppPos.r===getGoalRow('opp')) return -1e12;
  const beforeMy=shortestPath('me',state.walls,state.me);
  const beforeOpp=shortestPath('opp',state.walls,state.opp);
  const replies=[];
  for(const st of legalMoves('opp',n.walls,n.oppPos,n.myPos)) replies.push({type:'step',r:st.r,c:st.c});
  if(n.oppWalls>0){
    // For trap detection we deliberately scan ALL geometrically legal walls,
    // then retain every wall that worsens our route or creates a race threat.
    const maxR=getBoardRows()-2;
    for(let r=0;r<=maxR;r++) {
      if(performance.now()>=engineDeadline-80) break;
      for(let c=0;c<8;c++) {
        if(performance.now()>=engineDeadline-80) break;
        for(const o of ['h','v']){
          if(performance.now()>=engineDeadline-80) break;
          if(!wallGeometryLegal(r,c,o,n.walls)) continue;
      const copy=new Set(n.walls); copy.add(wallKey(r,c,o));
      const md=shortestPath('me',copy,n.myPos), od=shortestPath('opp',copy,n.oppPos);
      if(!isFinite(md)||!isFinite(od)) continue;
      const gain=md-beforeMy, enemyGain=od-beforeOpp;
          if(gain>=1 || (gain===0 && enemyGain<=0)) replies.push({type:'wall',r,c,o});
        }
      }
    }
  }
  let worst=Infinity;
  for(const r of replies){
    if(performance.now()>=engineDeadline-60) break;
    const q=applyMoveForSearch(r,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls,0,0,'opp');
    if(!q) continue;
    if(q.oppPos.r===getGoalRow('opp')) return -1e12;
    const md=shortestPath('me',q.walls,q.myPos), od=shortestPath('opp',q.walls,q.oppPos);
    if(!isFinite(md)) return -1e12;
    const myOptions=legalMoves('me',q.walls,q.myPos,q.oppPos).length;
    const opOptions=legalMoves('opp',q.walls,q.oppPos,q.myPos).length;
    const v=(od-md)*1500 + (myOptions-opOptions)*25 + (q.myWalls-q.oppWalls)*35;
    worst=Math.min(worst,v);
  }
  // Return the worst verified response. The caller uses this only as a
  // blunder guard; the full alpha-beta result remains authoritative otherwise.
  return worst===Infinity ? evaluateStateFull('me',n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls) : worst;
}

function chooseSafeRootMove(candidates, tactical){
  const ranked=candidates.slice().sort((a,b)=>rootTacticalScore(b)-rootTacticalScore(a));
  const pool=ranked.slice(0,Math.min(10,ranked.length));
  let best=tactical, bestRisk=-Infinity;
  for(const m of pool){
    if(performance.now()>=engineDeadline-80) break;
    const risk=rootTrapRisk(m);
    if(risk>bestRisk){bestRisk=risk;best=m;}
  }
  // If alpha-beta selected a move that is provably much more vulnerable to
  // an immediate wall trap than another top candidate, replace it.
  if(best && tactical){
    const tr=rootTrapRisk(tactical);
    if(bestRisk>tr+900) return best;
  }
  return tactical||best||ranked[0]||null;
}
...
function calculateBestMove(player){
  ...
  // With zero walls the entire remaining game is solved exactly by
  // retrograde analysis. This catches the precise class of position where
  // the old heuristic engine had already lost but kept searching.
  if(totalWalls===0){
    const exact=exactZeroWallMove();
    if(exact){
      engineStats.elapsed=performance.now()-start;
      engineStats.depth=999;
      engineStats.exact='zero-wall-retrograde';
      return exact;
    }
  }
  ...
  if(totalWalls<=MONSTER.endgameWalls){
    const deadline=Math.min(start+Math.max(ENGINE.timeMs,2200), performance.now()+Math.max(200,ENGINE.timeMs));
    engineDeadline=Math.min(engineDeadline,deadline);
    let best=null,bestVal=-Infinity;
    const moves=monsterMoveList(player,state.walls,state.me,state.opp,state.me.walls,state.opp.walls,48);
    const seen=new Map();
    exactNodes=0;
    for(const m of moves){
      ...
      if(!n) continue;
      const v=monsterExactEndgame(MONSTER.exactDepth,false,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls,-Infinity,Infinity,'opp','me',seen,new Set());
      if(engineAborted) break;
  ...
  return chooseSafeRootMove(root, tactical || root[0]);
}
...
function engineSelfTest(){
  ...
  try{
    ...
    gameMode="duel";
    const z=buildZeroWallSolver();
    const forcedWin=zeroWallOutcome(0*9+4,8*9+4,1);
    if(forcedWin!==1) failures.push("zero-wall retrograde win");
    if(!z || z.result.length!==2*81*81) failures.push("zero-wall table size");
