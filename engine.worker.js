...
/* ============================================================
   ...
   ============================================================ */
const MONSTER = { mctsMs: 360, maxChildren: 28, endgameWalls: 2, exactDepth: 30 };
...
function monsterMCTS(root, rootMoves, deadline){
  const rootIndex='me';
  const children=rootMoves.map(m=>({move:m,n:0,w:0,children:new Map()}));
  let total=0;
  const selectUct=(arr,parentN)=>{
    let best=arr[0],bestU=-Infinity;
    for(const ch of arr){
      const u=ch.n===0?Infinity:(ch.w/ch.n)+1.05*Math.sqrt(Math.log(Math.max(2,parentN))/ch.n);
  ...
  while(performance.now()<deadline){
    total++;
    const path=[];
    let node=selectUct(children,total); path.push(node);
    let n=monsterStateAfter(node.move,root.walls,root.myPos,root.oppPos,root.myWalls,root.oppWalls,'me');
    let turn='opp', depth=1;
    while(!n.winner && depth<16 && performance.now()<deadline){
      const moves=monsterMoveList(turn,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls, depth<4?12:7);
      if(!moves.length) break;
      // A shallow tactical selector replaces the old project's random rollout.
      let bestMove=moves[0], best=-Infinity;
      for(const m of moves){
        const x=monsterStateAfter(m,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls,turn);
        if(!x) continue;
        const v=monsterHeuristic(x.walls,x.myPos,x.oppPos,x.myWalls,x.oppWalls);
        const score=turn==='me'?v:-v;
        if(score>best){best=score;bestMove=m;}
      }
      n=monsterStateAfter(bestMove,n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls,turn);
      turn=turn==='me'?'opp':'me'; depth++;
    }
    let result;
    if(n.winner) result=n.winner.index===0?1:0;
    else result=1/(1+Math.exp(-monsterHeuristic(n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls)/140));
    // Backprop from root player's perspective.
    for(const ch of path){ch.n++;ch.w+=result;}
  }
  children.sort((a,b)=>b.n-a.n || b.w-b.w);
  return children[0] ? children[0].move : null;
}
...
function calculateBestMove(player){
  ...
  const root=monsterMoveList(player,state.walls,state.me,state.opp,state.me.walls,state.opp.walls,MONSTER.maxChildren);
  if(!root.length) return tactical;
  const close=[];
  const baseScore=tactical?monsterHeuristic(state.walls,state.me,state.opp,state.me.walls,state.opp.walls):0;
  for(const m of root){
    const n=monsterStateAfter(m,state.walls,state.me,state.opp,state.me.walls,state.opp.walls,'me');
    if(!n) continue;
    const v=monsterHeuristic(n.walls,n.myPos,n.oppPos,n.myWalls,n.oppWalls);
    // Do not let MCTS override a clearly tactical alpha-beta winner.
    if(!tactical || v>=baseScore-250) close.push(m);
  }
  if(close.length>1){
    const d=Math.min(MONSTER.mctsMs, Math.max(80, ENGINE.timeMs-(performance.now()-start)-20));
